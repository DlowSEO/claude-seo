...
```bash
curl -fsSL https://raw.githubusercontent.com/DlowSEO/claude-seo/main/requirements.txt \
...
For plugin installs, check `/plugin list` and reinstall `claude-seo@dlowseo-claude-seo`; subagents load from the plugin, not `~/.claude/agents/`.
